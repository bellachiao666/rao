...
from tests.helpers import make_tool_registry_with_mock_search


@pytest.mark.asyncio
async def test_default_tools_support_calculator_and_read_text_only(tmp_path):
    ...
    assert "not registered" in search.error


@pytest.mark.asyncio
async def test_mock_search_is_registered_from_test_helper():
    registry = make_tool_registry_with_mock_search()
